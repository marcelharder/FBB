...
export class GraphsComponent implements OnInit {
  ...
  genderModel: GraphModel = {
    DataXas: [],
    DataYas: [],
    DataFused: [],
    Caption: '',
  };
  outcomesModel: GraphModel = {
    DataXas: [],
    DataYas: [],
    DataFused: [],
    Caption: '',
  };
  countriesModel: GraphModel = {
    DataXas: [],
    DataYas: [],
    DataFused: [],
    Caption: '',
  };
  timingModel: GraphModel = {
    DataXas: [],
    DataYas: [],
    DataFused: [],
    Caption: '',
  ...
  ngOnInit(): void {
    ...
    // get gender data
    this.graph.getGender().subscribe({
      next: (data) => {
        this.genderModel = data;
      },
    });
    // get outcomes data
    this.graph.getOutcomes().subscribe({
      next: (data) => {
        this.outcomesModel = data;
      },
    });

    // get countries
    this.graph.getCountry().subscribe({
      next: (data) => {
    ...
    // get timing
    this.graph.getTiming().subscribe({
      next: (data) => {
        this.timingModel = data;
      },
    }); }

  showAge() {
    if (this.showGraphNo == 1) {
      return true;
    } else {
      return false;
    }
